// 全局变量
let selectedAPIs = JSON.parse(localStorage.getItem('selectedAPIs') || '["ruyi","mozhua","tyyszy","dyttzy", "bfzy"]'); // 默认选中资源
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!localStorage.getItem('hasInitializedDefaults')) {
        // 默认选中资源
        selectedAPIs = ["ruyi","mozhua","tyyszy", "bfzy", "dyttzy"];
...
function addAdultAPI() {
    ...
    if (!HIDE_BUILTIN_ADULT_APIS && (localStorage.getItem('yellowFilterEnabled') === 'false')) {
        ...
        adultTitle.innerHTML = `特殊资源采集站 <span class="adult-warning">
...
function checkAdultAPIsSelected() {
    ...
    if (hasAdultSelected) {
        ...
        if (filterDescription) {
            filterDescription.innerHTML = '<strong class="text-pink-300">选中特殊资源站时无法启用此过滤</strong>';
        }
        ...
        // 恢复原来的描述文字
        if (filterDescription) {
            filterDescription.innerHTML = '过滤"伦理片"等特殊内容';
        }
